...
class AnomalyDetectionService:
    ...
    def run(self) -> int:
        ...
        if len(cost_records) >= 10:
            ...
            if not raw_frame.empty:
                ...
                for (provider, scope, scope_key), group in analysis_frame.groupby(["provider", "scope", "scope_key"]):
                    ...
                    if ordered["cost_amount"].abs().max() < self.COST_SIGNAL_EPSILON:
                        continue
        ...
        anomalies_created += self._append_native_aws_anomalies()

        self.db.commit()
        return anomalies_created

    def _append_native_aws_anomalies(self) -> int:
        if not self.settings.aws_enabled:
            return 0
        try:
            native_items = AwsConnector().fetch_native_anomalies(self.settings.ingestion_lookback_days)
        except Exception:
            return 0
        for item in native_items:
            self.db.add(Anomaly(**item))
